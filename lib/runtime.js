...
export function transportSecurityWarning(baseUrl) {
  let parsed;
  try {
    ...
    return {
      level: "error",
      message: "Mautic baseUrl is not a valid URL. API credentials will not be sent until baseUrl is corrected.",
    };
  }
  ...
  if (parsed.protocol !== "http:") {
    return {
      level: "error",
      message: "Mautic baseUrl must use HTTPS in production, or plain HTTP only on a trusted private network.",
    };
  }
  const host = parsed.hostname.toLowerCase();
  const isPrivateName = TRUSTED_HTTP_HOSTS.has(host) || !host.includes(".");
  return {
    level: isPrivateName ? "warning" : "critical",
    message: isPrivateName
      ? "Mautic API credentials use Basic auth over plain HTTP. This is acceptable only on a trusted local or private Docker network; use HTTPS for production or any routable network."
      : "Mautic API credentials use Basic auth over plain HTTP to a routable host. Use HTTPS before sending production credentials.",
  };
}
...
export function getRuntimeConfig(api) {
  ...
  const baseUrl = stripTrailingSlash(nonEmptyString(pluginConfig.baseUrl) || nonEmptyString(process.env.MAUTIC_BASE_URL) || DEFAULT_CONFIG.baseUrl);
  return {
    baseUrl,
    ...
    transportSecurityWarning: transportSecurityWarning(baseUrl),
  };
}
